...
typedef struct W25Q_Chip {
	SPI_HandleTypeDef_flag *hspi_flag;			// SPI Handle
	...
	uint8_t			   		tx_buf[255];		// Transmit buffer
	uint8_t	   				rx_buf[255];		// Receive buffer
...
typedef enum ASYNC_W25Q_ReadStatusReg_State {
	ASYNC_W25Q_ReadStatusReg_READ_SR1 = 0,
	ASYNC_W25Q_ReadStatusReg_READ_SR2 = 1,
	ASYNC_W25Q_ReadStatusReg_READ_SR3 = 2,
} ASYNC_W25Q_ReadStatusReg_State;

typedef struct ASYNC_W25Q_ReadStatusReg_CONTEXT {
	W25Q_Chip *w25q_chip;
	ASYNC_W25Q_State state;
	// ASYNC_W25Q_ReadStatusReg_State read_status_state;
...
typedef struct ASYNC_W25Q_EarseSector_CONTEXT {
	W25Q_Chip *w25q_chip;
	uint32_t addr;
	bool is_ready;
	uint8_t tx_buf[4];
	ASYNC_W25Q_WaitAndProceed_State state;
} ASYNC_W25Q_EarseSector_CONTEXT;


typedef struct ASYNC_W25Q_EarseAll_CONTEXT {
	W25Q_Chip *w25q_chip;
	bool is_ready;
	uint8_t tx_buf[1];
	ASYNC_W25Q_WaitAndProceed_State state;
} ASYNC_W25Q_EarseAll_CONTEXT;
