...
void GPS_UART_Receive(GPS_t *GPS) {
    HAL_UARTEx_ReceiveToIdle_DMA(GPS->uart, (uint8_t *) rx_buffer, RX_BUF_SIZE);
    __HAL_DMA_DISABLE_IT(GPS->uart->hdmarx, DMA_IT_HT);
}
...
void GPS_UART_CallBack(GPS_t *GPS) {
    ...
    int i = 0, j;
    while (rx_buffer[i + 2] != '\0' && i + 2 < RX_BUF_SIZE) {
        memset(sentence, 0, GPS_BUF_SIZE);
        j = 0;
        while (rx_buffer[i] != '\r' && j < GPS_BUF_SIZE) {
            sentence[j] = rx_buffer[i];
            i++;
            j++;
        }
        sentence[j] = rx_buffer[i];
...
void GPS_USBPrint() {
    char buf[GPS_BUF_SIZE] = {0,};
    sprintf(buf, "UTC Time: %f, Latitude: %f, Longitude: %f, Altitude: %f\n", GPS.utc_time, GPS.dec_latitude, GPS.dec_longitude, GPS.msl_altitude);
